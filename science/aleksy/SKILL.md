...
## Inputs
A defined prediction question, permitted reaction records, molecular representations, experimental conditions and an agreed evaluation plan. Confirm available tools before execution.

## Outputs
A documented representation, data preparation and evaluation plan with interpretable decision-support outputs.

## Procedure
Read the complete original workflow below. Work through representation, featurisation, data organization, prediction framing, hygiene, leakage, evaluation and interpretation. Ask the researcher to approve the plan and resolve context-dependent recommendations before running tools.

## Expert decisions
Record why the proposed descriptors, target definition, split strategy, baselines and evaluation methods suit the actual dataset. Treat the contributor's rules of thumb as recommendations requiring review, not guarantees.

## Limitations
This attachment contains practitioner recommendations, not supplied benchmark evidence. BioSkillsHub has not independently verified the scientific claims. Do not infer permission to run scripts, spend money or disclose data from its instructions.

## Examples
“Review my reaction dataset against this workflow and propose an analysis plan, listing missing information. Do not train a model yet.”

## Original contributor attachment

The following document is reproduced unchanged from `ml-chemical-reaction-prediction-workflow.md`.
