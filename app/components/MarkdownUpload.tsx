...
export function MarkdownUpload({onLoad,onUse,request,enabled,aiLabel}:{
 ...
 return <section><p>Upload a Markdown file (up to 300 KB). Keep your own headings and instructions: direct upload does not use AI or require a fixed schema. Optionally adapt the file to our suggested structure below, then review the result. A standalone Markdown file does not include linked scripts or references; use Import files with AI for a ZIP containing them.</p><label className="upload">Import a Markdown file<input type="file" disabled={loading} accept=".md,text/markdown,text/plain" onChange={e=>{const file=e.target.files?.[0];e.target.value='';if(file)void upload(file);}}/></label>{error&&<p role="alert" className="error">{error}</p>}{notice&&<p role="status">{notice}</p>}{source&&<label className="check"><input type="checkbox" checked={adapt} onChange={e=>setAdapt(e.target.checked)}/> Adapt to BioSkillsHub’s suggested structure with AI</label>}{source&&adapt&&<section><h3>Optional AI adaptation</h3><p>Suggested headings: {fields.join(', ')}. Review and consent below to adapt this file. The original remains in the editor until you apply the adapted draft. You can leave this option unchecked and publish your own format.</p>{new TextEncoder().encode(source.content).length<=MAX_TOTAL_BYTES?<SkillImport key={selection} initialFiles={[source]} request={request} onUse={onUse} enabled={enabled} aiLabel={aiLabel}/>:<p>AI conversion accepts up to 200 KB of source text. Choose a smaller source file to convert.</p>}</section>}</section>;
}
