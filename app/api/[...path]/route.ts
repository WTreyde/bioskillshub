...
async function handle(request:Request,ctx:{params:Promise<{path:string[]}>}) {
 ...
 if(path[2]==='versions'&&path.length===4&&method==='GET')return json(await retrieve(user.id,id,integer.parse(Number(path[3]))));
 }
 ...
 const content=await generate(user.id,'draft',`Convert the expert answers to a Markdown skill. Use exactly these level-2 headings: ${fields.join(', ')}. Preserve scientific uncertainty and expert choices. Never invent parameter values, validation results or executable dependencies. No surrounding code fence. This is a draft for human review, not a verified protocol.`,{title:d.title,answers:d.answers});
