...
const directory=`results/agent-smoke/${randomUUID()}`;
...
try{
 ...
 execFileSync('python3',['scripts/agent_client.py','get','imagej-foci','1','--output',`${directory}/SKILL.md`],{env,stdio:'pipe'});
 const manifest=JSON.parse(await readFile(`${directory}/SKILL.md.manifest.json`,'utf8'));
