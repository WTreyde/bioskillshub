...
## Hide deployment verification entries

Added reversible administrator catalogue hiding by exact skill ID, with dry-run defaults and an additive `skills.hidden` migration. It preserves immutable releases and acquired pinned access, excludes hidden entries from discovery/recommendations and prevents new acquisition. The two user-specified deployment verification titles and apply/undo instructions are in `docs/team/wojtek.md`. Live cleanup is pending the integration administrator; no live data was changed here.
