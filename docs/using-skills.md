...
2. Choose files or a folder. A new selection replaces the previous selection. Supported UTF-8 text formats are Markdown, plain text, Python, R, JavaScript, TypeScript, JSON, YAML, TOML, shell and CSV. Limits: 20 files, 16 KB per file, 40 KB total. Archives and binary files are unsupported; hidden paths, node_modules, __pycache__, common secret filenames and unsafe paths are skipped. Large supported files cause an error; choose a smaller selection. If the browser cannot choose folders, use multiple-file selection.
