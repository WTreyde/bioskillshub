...
Deploy the merged landing/catalogue release from `origin/main`; no migration is required. Verify `/`, `/about`, `/browse` and GitHub sign-in on the public origin. Public password login controls were removed; operational credentials were not changed. All skills receive generated artwork automatically and new domains can be selected without seeding placeholder skills. Continue the separate Rosalind, Anthropic and ak importer handoffs if upgrading from PR #7.
