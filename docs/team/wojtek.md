...
- GitHub CLI was absent and a temporary CLI reported no GitHub authentication. Git author identity is unset in this clone; this follow-up uses the public Wojtek Treyde identity from the PR #1 merge commit for its commit only. Configure personal GitHub authentication before publishing changes and set a preferred Git author identity for future work; never reuse another teammate's credentials.
