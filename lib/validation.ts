...
export const draftSchema=z.object({title:z.string().trim().min(5).max(120),summary:z.string().trim().min(20).max(600),domain:z.enum(['Imaging','Chemistry','Genomics','Structural biology']),price_cents:z.number().int().min(0).max(100000),content:z.string().min(80).max(60000),validation:z.string().trim().min(5).max(200).default('Expert review pending'),release_notes:z.string().max(500).default('')});
