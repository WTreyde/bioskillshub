import {makeAgentSkill} from './agent-skill-format';
import { z } from 'zod';
...
export function scaffold(title:string, answers:Record<string,string>) {for(const f of fields) if(!answers[f]?.trim()) throw new Error(`Please complete ${f}.`);return makeAgentSkill(`# ${title}\n\n`+fields.map(f=>`## ${f}\n${answers[f]}\n`).join('\n'),title,answers['Use cases'].slice(0,1024));}
