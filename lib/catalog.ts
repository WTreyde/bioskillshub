...
export async function catalog(userId:string) {
 return query(`SELECT s.id,s.owner_id,u.name AS author,u.expertise,v.number,v.title,v.summary,v.domain,v.price_cents,v.validation,v.published_at,CASE WHEN v.eval_status='not_evaluated' AND demo.eval_passed THEN 'demo' ELSE v.eval_status END AS eval_status,
 (SELECT avg(stars)::float FROM skill_ratings WHERE skill_id=s.id) AS rating_average,
 (SELECT count(*)::int FROM skill_ratings WHERE skill_id=s.id) AS rating_count,
...
export async function saveDraft(userId:string,input:unknown,id?:string) {
 ...
 await c.query(`INSERT INTO drafts(skill_id,title,summary,domain,price_cents,content,validation,release_notes,eval_status) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)
 ON CONFLICT(skill_id) DO UPDATE SET title=$2,summary=$3,domain=$4,price_cents=$5,content=$6,validation=$7,release_notes=$8,eval_status=$9,updated_at=now()`,[skillId,d.title,d.summary,d.domain,d.price_cents,d.content,d.validation,d.release_notes,d.eval_status]);});return skillId;
}
export async function publish(userId:string,id:string) {
 await owned(userId,id);
 return transaction(async c=>{await c.query('SELECT id FROM skills WHERE id=$1 FOR UPDATE',[id]);const {rows:[d]}=await c.query('SELECT * FROM drafts WHERE skill_id=$1 FOR UPDATE',[id]);if(!d)throw new HttpError(409,'Save a draft before publishing.');validateContent(d.content,false);try{agentSkillMetadata(d.content);}catch(e){throw new HttpError(400,`Agent Skills format: ${(e as Error).message}`);}
 const {rows:[v]}=await c.query(`INSERT INTO versions(id,skill_id,number,title,summary,domain,price_cents,content,validation,release_notes,eval_status)
 SELECT $1,$2,COALESCE(MAX(number),0)+1,$3,$4,$5,$6,$7,$8,$9,$10 FROM versions WHERE skill_id=$2 RETURNING number`,[randomUUID(),id,d.title,d.summary,d.domain,d.price_cents,d.content,d.validation,d.release_notes,d.eval_status]);
