...
import {MAX_FILES,MAX_TOTAL_BYTES} from './upload-limits';
...
export function readImportUpload(name:string,bytes:Uint8Array):{files:ImportFile[];skipped:number}{
 if(bytes.length>MAX_TOTAL_BYTES)throw Error('Upload at most 200 KB.');
 ...
 if(count>MAX_FILES+100)throw Error('ZIP contains too many entries. Keep at most 100 files and 100 folders.');
