...
Creator studio also supports **Import files with AI** for a single UTF-8 source file in any programming language, or a ZIP-compressed folder containing scripts (1 MB ZIP container; 200 KB selected source contents). Review the selected files before sending them, then review the converted draft before saving/publishing. Original sources remain embedded as text in the Markdown download. See [file import instructions and limits](docs/using-skills.md#import-files-or-a-folder-with-ai).
